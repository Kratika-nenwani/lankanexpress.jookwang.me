...
<div class="container-fluid">
    <div class="row justify-content-center">
        <div class="col-12">
            ...
            <div class="row mt-4">
                <div class="col-lg-12">
                    <form id="callagent-form" method="POST" action="{{ route('saveamount') }}" class="d-flex align-items-center">
                        @csrf
                        <div class="form-group col-lg-4">
                            <select id="quote-select" name="quote_id" class="form-select" aria-label="Select Quote">
                                <option value="">Select TrackId</option>
                                @foreach($data as $quote)
                                <option value="{{ $quote->id }}">{{ $quote->id }} - {{ $quote->track_id }}</option>
                                @endforeach
                            </select>
                        </div>
                        &nbsp;
                        &nbsp;
                        <div class="form-group col-lg-4">
                            <input type="number" id="total" name="total" class="form-control" placeholder="Enter Amount">
                        </div>
                        &nbsp;
                        &nbsp;
    ...
    <div class="card" data-list='{"valueNames": ["orders-order", "orders-product", "orders-date", "orders-total", "orders-status", "orders-method"]}'>
        ...
        @foreach ($data as $quote)
        <div style="width:100%;" class="modal fade twm-sign-up" id="editModal{{ $quote->id }}" tabindex="-1" role="dialog" aria-labelledby="exampleModalLabel{{ $quote->id }}" aria-hidden="true">
            <div class="modal-dialog" role="document">
                <div class="modal-content">
                    ...
                    <div class="modal-body">
                        ...
                        <form id="editForm{{ $quote->id }}" action="{{ route('update-quote', ['id' => $quote->id]) }}" method="POST" enctype="multipart/form-data">
                            @csrf
