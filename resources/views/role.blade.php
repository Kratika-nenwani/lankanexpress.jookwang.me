        ...
        {{-- @foreach ($data as $plan)
            <div style="width:100%;" class="modal fade twm-sign-up" id="editModal{{ $plan->id }}" tabindex="-1"
                ...
                <div class="modal-dialog" role="document">
                    <div class="modal-content">
                        <div class="modal-header">
                            <h5 class="modal-title" id="exampleModalLabel{{ $plan->id }}">Edit city</h5>
                            <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
                        </div>
                        <div class="modal-body">
                                        ...
                                        <div class="row">
                                          ...
                                          <div class="col-lg-12">
                                            <div class="form-group mb-3">
                                                ...
                                                <input name="benefits" type="text" id="benefits" class="form-control"
                                                    value="{{ is_array($plan->benefits) ? implode(', ', $plan->benefits) : $plan->benefits }}" required>
                                            </div>
                                        </div>
...
<script>
  $(document).ready(function() {
      ...
      $(document).on('click', '.operation_btn_ajax', function() {
          var userId = $(this).data('id');
          $.ajax({
              url: "{{ route('assign-operation') }}", // Route for handling operation assignment
              type: 'POST',
              data: {
              ...
              success: function(response) {
                  ...
                  Swal.fire({
                      icon: 'success',
                      title: 'Success',
                      text: 'Operation assigned successfully!',
                      confirmButtonText: 'OK'
                  }).then(() => {
              ...
              error: function(xhr) {
                  ...
                  Swal.fire({
                      icon: 'error',
                      title: 'Error',
                      text: 'Error assigning operation!',
                      confirmButtonText: 'OK'
                  });
              }
          });
      ...
      // Handle Callagent button click
      $(document).on('click', '.callagent_btn_ajax', function() {
          var userId = $(this).data('id');
          $.ajax({
              url: "{{ route('assign-callagent') }}", // Route for handling callagent assignment
              type: 'POST',
              data: {
                  _token: "{{ csrf_token() }}", // CSRF token
                  user_id: userId
              ...
              success: function(response) {
                  ...
                  Swal.fire({
                      icon: 'success',
                      title: 'Success',
                      text: 'Callagent assigned successfully!',
                      confirmButtonText: 'OK'
                  }).then(() => {
                    window.location.reload(); // Reload the page to reflect changes
                  });
              ...
              error: function(xhr) {
                  ...
                  Swal.fire({
                      icon: 'error',
                      title: 'Error',
                      text: 'Error assigning callagent!',
                      confirmButtonText: 'OK'
                  });
              }
          });
